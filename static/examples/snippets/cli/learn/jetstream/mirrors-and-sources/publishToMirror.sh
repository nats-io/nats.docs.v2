...
# aimed at the mirror name reaches no stream. A JetStream publish is a
# request; with nothing listening on the subject, the server returns an
# immediate "no responders available" status, so the publish fails right away.
...
# Published 91 bytes to "ORDERS-ARCHIVE"
