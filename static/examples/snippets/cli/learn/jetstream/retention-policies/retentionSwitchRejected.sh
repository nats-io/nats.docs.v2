...
# at creation. Try to move JOBS (WorkQueue) to Limits and the server
# refuses:
#   stream configuration update can not change retention policy to/from
#   workqueue
# The command exits non-zero — JOBS keeps its WorkQueue policy.
nats stream edit JOBS --retention limits --force
