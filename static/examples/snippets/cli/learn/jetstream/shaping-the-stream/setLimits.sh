...
#
# `nats stream edit` changes an existing stream in place. The messages
# already stored stay where they are; only the configuration changes. The
# command prints a diff of what will change and asks for confirmation
# before applying it.
#
# --max-age sets MaxAge: the oldest a message may get. 7d means a
#   message is removed roughly seven days after it was stored.
# --max-bytes sets MaxBytes: the most disk the stream may occupy.
#   1GiB caps it at one gibibyte (about a gigabyte); the stream
