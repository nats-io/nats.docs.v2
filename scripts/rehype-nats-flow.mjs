...
const FALLBACKS = {
  ...
  streamSnapshotAnimated:
    "Backing up the ORDERS stream with nats backup stream. The backup client sends a snapshot request to the ORDERS stream and opens an ephemeral inbox to receive the data. The server answers with the stream's config and state, then drains its store as an S2-compressed archive and streams stream.arc.s2 to the inbox as a sequence of chunks. The client returns a flow-control ack per chunk; the server keeps a window of unacknowledged chunks in flight and uses the acks as backpressure so a slow disk or link can't be overrun. When the last chunk is acked, backup.json (config and state) and stream.arc.s2 (the messages) land in the off-site backup store.",
  mirrorDRAnimated:
