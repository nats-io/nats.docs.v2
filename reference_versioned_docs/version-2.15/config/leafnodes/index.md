...
| [`dial_timeout`](./dial_timeout.md) | How long to wait for the TCP connection to a remote server to be established. Raise it on high-latency links where the default is too short for the handshake to complete. Applies to every remote, unless a remote sets its own `dial_timeout`. | `duration` | `1s` | No |
