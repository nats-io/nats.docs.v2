...
| [`dial_timeout`](./dial_timeout.md) | How long to wait for the TCP connection to this remote to be established. Overrides the server-wide `leafnodes.dial_timeout` for this remote only. If not set, the server-wide value applies, which itself defaults to `1s`. | `duration` | - | No |
