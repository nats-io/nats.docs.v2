...
| [`max_concurrent_io`](./max_concurrent_io.md) | Maximum number of disk I/O operations the server allows in flight at once, across all streams and consumers. Must be between 4 and 8192. Lower it to reduce contention for the underlying disk on a server with many assets; raise it on fast storage to allow more parallel I/O. | `integer` | `4096` | Yes\* |
