...
version, and a built-in identity. The bytes on the wire do not change.
...
page does not re-teach how a reply finds its way back — it teaches the
...
and read its stats. You do not see them here; the
...
That is the whole shape: `Data()` in, `Respond()` out. The framework took
...
the response flows back to the caller's reply subject. Here is the round
...
green arrow is the answer coming back. Underneath it is plain
...
validation: the framework names and wires your responder, but it does not
...
must match the framework's character rule (letters, digits, hyphen,
underscore — no dots or spaces), and the `Version` must be valid SemVer.
...
Nothing about the payload or the subject changed. The responder simply
became a service.
