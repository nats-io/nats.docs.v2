...
service discoverable and observable without any extra wiring. Ask `$SRV.INFO`
...
one of them. There is no coordinator and no config — scaling is "run more."
...
schema of an INFO response, that is where to look.
...
moment your handler needs durable state — an order that must survive a
...
healthy local connection; production connections are not always healthy.
...
them gracefully — each `Stop()` drains in-flight requests and unsubscribes the
discovery verbs — when you are done.
...
is the floor for everything else you will build on the framework.
...
- [ ] Pick a `Name` that matches the allowed character set and a `Version` that is valid SemVer; an invalid value fails service creation outright.
...
- [ ] Treat endpoints and metadata as immutable once added; there is no remove, so name and shape them deliberately the first time.
