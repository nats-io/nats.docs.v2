...
A mirror is read-only. You can't publish to it directly, because it has
...
new stream isn't a normal stream but a mirror of `ORDERS`. You don't
give it `--subjects`, because a mirror captures nothing of its own.
...
the mirror can't keep pace with the upstream's write rate.
...
different upstreams there's no ordering guarantee; the merged stream
...
needed. That flexibility is the practical difference from a mirror.
...
are *services* (they're request-reply), while delivery is a *stream*
...
`nats pub` aimed at `ORDERS-ARCHIVE` doesn't error with "this is a
...
and its `Lag` climbs above `0`. Don't assume a message in `ORDERS` is
...
exports (they're request/reply), while the delivery subject is a *stream*
export (messages flow one way). Get a type wrong and replication doesn't
error loudly; the mirror just never catches up. Don't eyeball the
