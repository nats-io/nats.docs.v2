...
in stock?_ That is a request and a reply, not a broadcast.
...
Request-reply is not a new protocol. It is the pub/sub you already
know, used twice.

Here is the whole dance. The client invents a fresh, unique subject
...
The orange arrow is the request travelling out on
`orders.inventory.check`. The green dashed arrow is the reply
travelling back on the private subject the client made for this one
call.
...
The private reply subject has a name and a convention. It is called an
...
The `_INBOX.` prefix is reserved precisely so it does not collide with
...
There is a size budget on the reply subject. The server limits the
...
Leave that running. It is now the one service in the Acme world that
...
running unchanged — request-reply does not replace them, it adds a new
...
or hold it for later — that is the at-most-once guarantee from the
publish-subscribe page, and it applies to replies too. A reply that
does not arrive is simply gone.
...
timeout on every `request()` call — there is no infinite wait to fall
into by accident.
...
the caller. Core NATS will not make that decision for you, and it will
not deliver the answer late.

A timeout tells you the answer did not arrive in time. It does not
...
get a timeout after 2s) and "the inventory service is not running at
...
You will not need them for the inventory call, so this page does not
...
that is not part of the business payload — a request ID, a trace
...
exactly one of them to handle each request? That is a **queue group**,
...
milliseconds — it is not a slow timeout. Branch on it separately:
no responders means the inventory service is not deployed, while a
timeout means it is deployed but slow. Lumping them together hides a
...
never see it and you never know it existed. When more than one service
...
When you actually want every responder to answer, that is
[scatter-gather](/learn/core-nats/scatter-gather), not a bug. When you
want exactly one of several instances to handle each request, that is a
...
That is a queue group — built-in load balancing with no broker in the
