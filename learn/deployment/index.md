...
It is the runbook, not the theory. Each page names the mechanism, gives
the exact command or manifest the operator runs, and links the *why* out
...
A production runbook for the same Acme ORDERS cluster you have followed
...
- The cluster **running on Kubernetes** as a three-replica StatefulSet
  ...
  states that mapping), with the `ORDERS` stream and the `shipping` and
  `analytics` consumers declared as CRDs.
...
You are an operator standing the cluster up for real. You have read the
[Topologies deep dive](/learn/topologies) and know the shapes, and you
have skimmed the [JetStream](/learn/jetstream) and
...
- **How Raft and replication work underneath** is
  ...
  transfer during an upgrade; it does not explain how Raft elects a
  leader.
...
- **What to watch once it is live** is [Monitoring](/learn/monitoring).
...
full set of server configuration options is documented in
...
| 7 | [Where to go next](/learn/deployment/where-next) | The production checklist and a map of what is beyond running the cluster |
...
- [Clustering & Replication](/learn/clustering) — the Raft mechanics this
  chapter triggers but does not teach.
