...
Sizing a NATS node is not guesswork. A node spends a small, fixed set of
...
It teaches two things: the **four resources** a node spends and their
JetStream defaults, and **account limits** — how the server counts R3
replication against the storage ceiling.

## The four resources a node spends

Every NATS node spends the same four resources. Size each one and you
have sized the node.

**CPU** moves messages. Core NATS routing is cheap; TLS handshakes and
JetStream replication are where cycles go. There is no hard CPU limit to
...
stream actually spends, and the one most likely to run out. We size it
below.
...
JetStream spends roughly **2 FDs per stream**. On a small cluster the
default per-process limit is plenty; on a large one it is not, which is
...
When JetStream cannot read the system's real limits, it falls back to two
...
what the streams actually store. But it is a *trap* in a container: the
...
regardless of how much disk the node has. Read them live before you size:
...
The durability that R3 buys — surviving the loss of a node — is the
...
unlimited; it is reloadable, and overflow disconnects immediately).
...
A few sizing mistakes only surface in production, under load, when they
are expensive to fix. Each is scoped to this page's two concepts — the
...
cannot store. The failure is a publish error mid-stream, not a startup
warning. Set `max_file_store` to the volume size, test small with a
`10GB` value, and watch the real disk with `df -h`. Do not trust the
...
`≥ 10× peak message size` so a burst of large messages cannot stall a
connection. Do not raise `max_payload` in isolation.

**File-descriptor exhaustion on a big cluster.** Each stream costs about
2 FDs, and routes and gossip add more. On a large cluster the default
...
enforce their limits through the account JWT; pre-v2.10 servers did not
