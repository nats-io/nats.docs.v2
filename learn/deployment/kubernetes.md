...
named by ordinal: **`nats-0`, `nats-1`, `nats-2`**. They are the same
...
This page introduces two ideas. The first is the **NATS Helm chart**,
which deploys the cluster as a StatefulSet. The second is the **NACK
controller**, which lets you declare the `ORDERS` stream and its
consumers as Kubernetes resources instead of running CLI commands.

## The NATS Helm chart deploys a StatefulSet

A NATS cluster is not a stateless web app. Each node owns a slice of the
...
The full set of Helm values is large; the chart ships sensible defaults
for almost all of it. The full set of server configuration options is
documented in [Reference → Configuration](/reference/config). We only
cover the values this deployment needs here.
...
may need to catch its stream replicas up from its peers before it is
useful, and that can take minutes — the generous failure threshold keeps
Kubernetes from killing a pod that is doing legitimate startup work. The
**readiness probe** asks whether this server is ready to serve clients,
so a not-ready pod is taken out of the service rotation. The **liveness
probe** asks only whether JetStream is enabled at all; it is the last
resort that restarts a truly wedged process.
...
stream add ORDERS` by hand. That works, but it is imperative: the stream
exists because someone ran a command, and nothing brings it back if it is
...
Here is the `ORDERS` stream as a CRD — the same R3 file stream from every
...
The reconcile is a closed loop, and that is the payoff. If someone
...
cluster is continuously steered back to it.
...
[Reference → Configuration](/reference/config). We only set the fields
the ORDERS stream needs here.
...
cannot start until its persistent volume is bound, and on a cluster with
...
controller sees the drift and reverts your change within about thirty
...
missing `replicas: 3`, or the controller has not finished reconciling —
...
- Three pods — `nats-0`, `nats-1`, `nats-2` — managed by a StatefulSet,
  each with a stable name, DNS entry, and volume. They are the same
...
cannot yet change without restarting pods. The next page splits that
