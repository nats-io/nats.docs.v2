...
A runbook starts before the outage. The decision you do not want to make at
...
| Someone deleted or purged `ORDERS` by mistake | restore the snapshot | The mirror followed the delete — it is gone there too. The snapshot is the only intact copy. |
...
faithfully copies your errors too. That is why the chapter built both, and
why [R3 replication](/learn/clustering) is on neither row — replicating a
bad write three times does not undo it.

## Failover: promote the mirror

When the `east` cluster is gone, the goal is to make `ORDERS_DR` start accepting
writes so `order-svc` and the consumers can carry on at `site2`. That is
**promotion**, and it is a short, ordered sequence. The animation below
walks it end to end.
...
A mirror trails its upstream by some **lag** — the count of messages it has
not yet copied. Promote it while lag is non-zero and you start publishing on
top of a stream that is still missing its tail. So the first runbook step is
...
If the `east` site is fully dark, the mirror cannot reach its upstream and the lag
...
holds every message it had copied — promotion does not touch the data, only
the relationship.
...
`orders.cancelled` — the same subjects the lost primary held. It is a full
...
The other rows of the table do not fail over — they roll back. When `ORDERS`
...
For an accidental delete, restore straight from the snapshot — do not
...
Here it is one step in a larger procedure.
...
all three copies at once. R3 is availability, not a backup. Do not list it on
the mistake rows of the table — that is precisely what snapshots are for.
...
behind you, and you purge forever chasing a moving tail. Quiesce the
...
time you run it is not during the outage.
...
copied the mistake. R3 is on neither path; it is availability, not recovery.
