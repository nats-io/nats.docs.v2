...
messages from 1. The flag worth knowing about before you need it is
`--obfuscate`: it replaces names, subjects, and message bodies with keyed
hashes and zeros of the same length, so you can hand a support case a
backup shaped like your production data without the data in it. The key
file it writes beside the target turns the hashes back into real values
through `nats backup lookup`.
