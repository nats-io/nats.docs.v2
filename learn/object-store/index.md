...
NATS gives you a place to store files, and it is not a separate service. An
...
There is a sibling store you should place this next to. The
...
large chunked files and only the latest metadata per object — it remembers
...
"a stream stores messages" already means something to you. This chapter does
not re-teach what a stream is, how a consumer tracks its position, or how
acknowledgment works. Where you would want those, it names the gap and links
back to [JetStream](/learn/jetstream).

You do not need to know anything about the object store specifically. We
start from "what is a bucket and why would you want one" and grow from there.

## How to read it

Each page introduces at most two new concepts. Pages build on the previous
one: the same `INVOICES` bucket is used throughout, and you can keep one
terminal open through the whole chapter without resetting state. You create
the bucket and store your first invoice on page 1, store a large multi-chunk
invoice on page 2, attach metadata and a link on page 3, list and watch the
bucket on page 4, and inspect the stream underneath on page 5.
...
what you need to understand the concept. The full set of object store
configuration options is documented in [Reference](/reference/). We only
...
| # | Page | What you learn |
|---|---|---|
| 1 | [Your first object](./your-first-object) | Create `INVOICES`, put `invoice-ord_8w2k.pdf`, and get it back with its digest verified |
| 2 | [Chunking](./chunking) | Store a 3 MB invoice across many chunks and read the chunk count |
| 3 | [Metadata and links](./metadata-and-links) | Attach a description, headers, and a metadata map, then link one object to another |
| 4 | [Watching and listing](./watching-and-listing) | List the bucket as a snapshot, then watch it for new objects live |
| 5 | [Under the hood](./under-the-hood) | See the `OBJ_INVOICES` stream, the chunk and metadata subjects, and rollup versus soft delete |
| 6 | [Where to go next](./where-next) | A map of what is beyond objects, and one pre-production checklist |
