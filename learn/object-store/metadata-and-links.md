...
ask "what's this?" without paying for the bytes.
...
There's one boundary worth naming. The metadata describes the *current*
...
Two rules keep links sane, and the store enforces both. A link can't point
at a deleted object, and a link can't point at another link — the store
refuses to build a chain you'd have to chase. When you add a link, the
...
Metadata and links come with two traps: what a link does when its target
moves, and what `UpdateMeta` will and won't change.

**A link is a snapshot, not a live reference.** Adding a link records the
target's bucket and name at creation time; it doesn't pin the target alive.
...
The bucket now holds an object that can describe itself, and a link beside
it. The next page steps back from single objects to the whole bucket: how to
