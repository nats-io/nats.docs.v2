...
service does not know the names ahead of time — it wants to discover what
is in the bucket, and to learn the moment a new object lands.
...
it. These are the two ways to read a bucket without already knowing what is
inside it.

## List is a snapshot

A **list** returns every non-deleted object in the bucket as a one-time
snapshot. It does not stay open and it does not stream changes — it answers
the question "what is in here now?" and returns.
...
only what is really there. The deletion mechanism itself lives on
[Under the hood](/learn/object-store/under-the-hood); here, the behavior
is all you need: list shows live objects, nothing more.

An empty bucket is not an error. Listing a freshly created bucket returns
...
bucket changes — a new put, a re-put, a delete. It is how `analytics`
...
to say "you are now current; everything after this is a live change." The
...
That update carries the new object's *metadata* — its name, size, chunk
count, digest — and nothing else. The bytes are not on the watch. This is
the single most important fact about watch, and the next section is built
entirely around it.
...
This is by design, and it is what makes watch cheap. `analytics` can watch
...
with no live objects does not fail — it returns zero entries. In the
client libraries this surfaces as a distinct "no objects found" condition,
and code that treats it as a fatal error will crash on a bucket that is
simply empty, or one whose objects were all deleted. Do treat an empty
...
You have now used the bucket from every angle — put, get, chunking,
