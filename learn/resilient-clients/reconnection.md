...
the `n1`/`n2`/`n3` cluster. A naive client just dies there.

This page makes the connection survive that. When the link drops, the
client doesn't give up: it cycles the server pool, waits a little
between tries, and rejoins. While it's away, the publishes the
application keeps making don't vanish; they queue and flush once the
...
The client doesn't hammer the pool. After it has tried every URL once
...
against a pool that isn't ready yet.
...
the publishes the application makes *while the link is down*?
...
other. The buffer survives the gap; it doesn't turn a publish into a
...
the default backoff with a custom delay of zero, the client spins a
tight CPU loop against a pool that isn't ready. If you make it a fixed
delay with no jitter, a fleet of clients that all lost the same server
retry in lockstep and hammer the one survivor at the same instant.
Always keep a non-zero wait, and always keep jitter: that's exactly
what the herd spreads across.
...
overflows it, and the publish that overflows fails. Don't treat that
...
disconnect is usually instant, but a connection that's merely wedged
(the server alive but not reading) is only caught by the
...
link is detected in seconds, not minutes. The full set of keepalive
options is in [Reference](/reference/).
