...
production both of those are unacceptable.
...
an nkey seed, a private key the client uses to sign a challenge. You do not
...
The creds file is the whole identity. There is no `--user` and no
`--password` — the JWT names the user and the seed proves it. That also means
the file is a secret: anyone who holds it can connect as `order-svc`, so it is
loaded from disk or an environment, never pasted into source.
...
it does not, the client aborts before sending any credentials.
...
There is a mirror-image option in one line. So far the CA lets the client
...
The full set of TLS and credentials options is documented in
[Reference](/reference/). We cover only the CA the client trusts and the creds
it presents here.
...
into the application gets committed, shared, and leaked, and it cannot be
...
encrypted link is worthless if you cannot trust who is on the other end.
...
unnoticed — but the moment the connection drops and the client tries to
reconnect, authentication fails and the connection cannot recover. The failure
...
request-resilient, and now authenticated over TLS. That is the full client
state machine, hardened.

## What is next

That is every mechanism this chapter adds to the Acme clients. The last page
