...
You have secured the ORDERS world end to end. This page does two things.
It puts the whole model back together in one picture. Then it points you
at the chapters and references that go past where this one stops.

## The whole game in one picture

Every page in this chapter added one piece. Here they are as a single
sentence.
...
each answer is configured.

## The two ways to answer "who are you?"

The one fork worth re-stating is authentication. The chapter taught it
twice on purpose.

**Centralized authentication** keeps the user list in the server's own
config. The server checks a credential against that list directly. It is
...
only the operator's public nkey. It is the right tool when you have many
tenants, or when you do not want to restart the server to add a user.
...
question this chapter pointed at but did not unpack: what happens to the
...
If you want to revisit one piece, here is the chapter in order:
...
any NATS connection. You have built the same two-account ORDERS world
twice, once in centralized config and once under an operator, and you
have shared exactly one subject across the boundary on purpose.

That is the whole model. The chapters above take it to many servers, many
