...
`--replicas=3` is the whole change. The application code does not move:
...
stream leader, `n1-east`. The leader does not store it and immediately
...
The third peer is not on the critical path. `n3-east` may ack a moment
later, or be briefly behind; the write committed without waiting for it.
That is the point of a quorum — the group makes progress as long as a
majority is reachable, not all of it.

## Followers apply what the leader commits

Committing records that a quorum *has* the entry. It does not yet put
...
Here is one write from `order-svc` moving through that whole sequence —
...
The full set of RAFT replication parameters — append-entry batching,
heartbeat intervals, log compaction — is documented in
[Reference](/reference/system/monitor/raftz). We only need the
append → quorum → commit → apply shape here.
...
returns, a read from the leader sees that order. There is no window
...
This is the trade `R=3` makes on purpose. It does not promise that every
...
There is no second peer, so there is no quorum and nothing to commit
...
Do not run real orders at `R=1`; the why-three reasoning lives on
...
return data that is correct but not the newest. Do not assume any peer
...
moment. That is correct and safe: the write already survives one node
loss. But do not read a `PubAck` as "all three copies are identical
...
- A write **appends** to the leader's log, replicates as an **append
  entry** to followers, and **commits** once a **quorum** holds it — two
  of three for `R=3`.
...
landed. The next page makes that choice yours: **placement** — constraining a
stream's replicas to a cluster and to servers carrying matching tags,
and hinting which peer should lead first.
