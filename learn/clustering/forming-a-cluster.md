...
This page opens that box. It is the first mechanism of the chapter, the
...
the end of the page they are running and have discovered themselves from
a single seed.

## A route is a server-to-server connection

A **route** is the connection one `nats-server` opens to another so the
two act as one cluster. It is not a client connection. A client connects
...
how a cluster fills in the connections you did not configure — and they
come from gossip.
...
`n3-east`. `n2-east` had no route to `n3-east`, so it opens one — an
...
`n3-east` repeats it, one port higher, pointing at the same seed. Note it
does **not** list `n2-east` — it does not need to. Gossip will hand it
that route.
...
[Reference → Cluster config](/reference/config/cluster). We need only
...
`name: eest` on the third, and the third does not error — it forms its own
one-server cluster and never merges. You are left with two clusters that
...
unavailable at boot. Do not rely on a single seed in production:
...
of them dies. That is RAFT, and it is the next page.
