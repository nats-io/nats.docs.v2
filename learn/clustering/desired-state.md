...
Replacement here is **best effort**. The job is to clear the server, so an
asset the meta leader can't place elsewhere — nothing satisfies its placement,
or the cluster is too small once `n4-east` is out — still moves off `n4-east`
and carries on **under-replicated** until a qualifying server appears. An `R=3`
stream can end up running on two peers. The server logs
...
Peer *removal* is a different operation. Removing a peer drops it from a
group without migrating anything onto a replacement first. That is what you
want for a server that is gone and isn't coming back, which is the
...
If you change your mind mid-operation and want to cancel the move, you can with:
...
Two checks run before anything happens, and they're how you find out
