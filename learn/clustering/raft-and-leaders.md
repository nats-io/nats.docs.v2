...
forward messages. But knowing about each other is not the same as *agreeing*
with each other.
...
A cluster does not run one RAFT group. It runs many, layered.

The first is the **meta group**: one cluster-wide RAFT group whose peers are
the servers themselves. Its log is not your data — it is the cluster's
...
across its three peers. They are independent. The meta leader and the `ORDERS`
...
The `Cluster Information` block names the stream's RAFT group — its `Leader`
and each `Replica` peer:
...
`Leader` is the stream leader. Each `Replica` line is a follower peer, whether
...
A group has one leader at a time. When the leader is healthy, it is simple:
...
hung, or got cut off by the network. The followers cannot tell *why* the
heartbeat stopped, only that it did. So they do not wait forever. Each follower
...
An election needs one more idea: the **term**. The term is a number that counts
elections — a monotonically increasing count. Every entry and every
vote is stamped with a term, so the group can always tell a stale message from
a current one. A leader from an older term is automatically obsolete the moment
a newer term exists.

Here is the sequence when a follower's election timer fires.

The follower becomes a **candidate**, the third RAFT role. It increments the
term to one higher than any it has seen, votes for itself, and asks every other
peer to vote for it in this new term.

Each peer grants its vote if it has not already voted in this term and the
...
group that is two: the candidate's own vote plus one more. With the majority in
...
the survivor cannot reach a majority, so it cannot become leader and the group
...
You do not have to take this on faith. With `east` running and `ORDERS`
...
(2 of 3), so they elected a new leader and `ORDERS` is writable again, even
with `n1-east` down.
...
RAFT is robust, but its timing and its layering surprise people. Each trap
...
between 4 and 9 seconds after the last heartbeat, deliberately staggered so two
followers do not become candidates at the exact same instant. So when you kill
a leader, expect a short window where `nats stream info` shows no leader and
writes are refused. That window is RAFT working as designed, not a bug. Do not
build a client that treats a brief "no leader" as a fatal error — let it retry,
because a leader is seconds away.

The right response is to retry the write, not to fail. A `nats stream info`
during the gap confirms what is happening:
...
still chosen by a normal quorum election among the remaining peers. There is no
flag that hands leadership to a specific server. Do not run stepdown expecting
...
leader does not lose the `ORDERS` stream leader, and vice versa — they are
...
- You have killed a leader and watched the survivors run a **leader election** —
  a candidate, a bumped **term**, and a **quorum** of votes — and you have moved
  a leader on purpose with **stepdown**.

What you have not done yet is follow a single write through the group: how the
leader gets an order onto all three peers and decides it is safe.
